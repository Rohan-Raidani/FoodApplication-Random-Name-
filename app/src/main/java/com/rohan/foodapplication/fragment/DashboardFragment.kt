...
import com.rohan.foodapplication.R
...
class DashboardFragment(val contextParam: Context) : Fragment() {
    ...
    override fun onCreateView(
       ...
       // val radio_low_to_high = view.findViewById(R.id.radio_low_to_high)
            ...
            R.id.sort -> {
                ...
                AlertDialog.Builder(activity as Context)
                    ...
                    .setPositiveButton("OK") { _, _ ->
                        if (radio_high_to_low.isChecked) {
                            Collections.sort(restaurantInfoList, costComparator)
                            restaurantInfoList.reverse()
                            dashboardAdapter.notifyDataSetChanged()
                        }

                        if (radio_low_to_high.isChecked) {
                            Collections.sort(restaurantInfoList, costComparator)
                            dashboardAdapter.notifyDataSetChanged()
                        }
                        if (radio_rating.isChecked) {
